...
//         Hand changes by 0 (b/c of playing Great Hall)
//         Deck+Discard count decreases by 1
//         Great hall card goes to played pile
//         Action Count increases by 1
int testGreatHallCaseA(int iterations) {
  ...
  for (z = 0; z < iterations; z++ ) {
    ...
    // result = playGreatHall(&postGame, handPos);
    ...
    // Action Count increases by 1
    // TEST 8
    test8_string = "Expected numActions to be 1 more.";
    result = ( preGame.numActions + 1 == postGame.numActions );
    if ( result == 0) {
      if (DEBUG_FLAG) printf("\t%s\n", test8_string);
      didFail = 1;
      test=8;
...
//         Great hall card goes to played pile
//         Action Count increases by 1
int testGreatHallCaseB(int iterations) {
  struct gameState preGame;
  struct gameState postGame;
  int deckSize;
  int discardSize ;
...
//         Great hall card goes to played pile
//         Action Count increases by 1
int testGreatHallCaseC(int iterations) {
  struct gameState preGame;
  struct gameState postGame;
  int deckSize;
  int discardSize ;
...
int testGreatHall( int iterations ) {
  // Great Hall
  // Result: Player gains 0 or 1 cards from its deck
  //         Hand changes by -1 or 0 (b/c of playing Great Hall)
